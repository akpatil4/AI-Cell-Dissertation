...
IMG_SIZE_X = 870
IMG_SIZE_Y = 419 # based on img size Y of 870
...
################################### contour logic/completing ###############################################################################################################################################################
                ...
                if len(contour_list) != 0:
                    if np.linalg.norm(contour_list[-1]-final_path_arr[0]) <= 20: # if new path start is close to old path end
    ...
    if st.button("Export Contour") and loop_closed:
        export_array = np.array(contour_list)/SCALE + np.array([state.tl_x, state.tl_y])
...
with col1:
    if st.button("Erase Contours"): 
        contour_df = pd.DataFrame(columns=["x","y","contour_id"])
        contour_df.to_csv("contour.csv",index = False)
    ...
    for i in list(contour_df["contour_id"].unique()):
        ...
        cv2.drawContours(img_overlay, [np.array(current_contour.drop(columns="contour_id"))], -1, (0, 255, 0), 2)
